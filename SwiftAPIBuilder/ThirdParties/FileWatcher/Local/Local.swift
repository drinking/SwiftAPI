...
public extension FileWatcher {
    ...
    public final class Local: FileWatcherProtocol {
        ...
        public func start(closure: @escaping FileWatcher.UpdateClosure) throws {
            ...
            try startObserving(closure: closure)
        ...
        private func startObserving(closure: @escaping FileWatcher.UpdateClosure) throws {
            ...
            let source = DispatchSource.makeFileSystemObjectSource(fileDescriptor: handle, eventMask: DispatchSource.FileSystemEvent.all, queue: queue)
            ...
            source.setEventHandler {
                let flags:DispatchSource.FileSystemEvent = source.data
                if flags.contains(.delete) || flags.contains(.rename){
                    _ = try? self.stop()
                    _ = try? self.startObserving(closure: closure)
                    return
                }
        ...
        public func refresh() {
            ...
            guard let content = try? NSData(contentsOfFile: path, options: .uncached) else {
                isProcessing = false
                return
            }
        ...
        private func throttle(after: Double, action: @escaping () -> Void) -> CancelBlock {
            ...
            let timeInterval = DispatchTime(uptimeNanoseconds: UInt64(after*Double(NSEC_PER_SEC)))
